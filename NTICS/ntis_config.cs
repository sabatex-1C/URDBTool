...
namespace NTICS
{
    public class ntics_config
    {
        ...
        public static void SaveProperty(String Sender, string PropertyName, string Property)
        {
            ...
            XmlNode node = ntics_config.XML.DocumentElement.SelectSingleNode("descendant::" + Sender);
            if (node == null)
            {
                node = ntics_config.XML.DocumentElement.AppendChild(ntics_config.XML.CreateNode(XmlNodeType.Element, Sender, ""));
            }
            ...
            if (atr == null)
            {
                atr = ntics_config.XML.CreateAttribute(PropertyName);
